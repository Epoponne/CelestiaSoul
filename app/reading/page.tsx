  ...
  return (
    <main style={{background:'#06050E',minHeight:'100vh',color:'#E8E0FF',fontFamily:'Georgia,serif'}}>
      <div style={{maxWidth:'680px',margin:'0 auto',padding:'0 18px 60px'}}>
        ...
        <div style={{display:'flex',gap:'0',marginBottom:'24px',background:'rgba(255,255,255,0.02)',border:'1px solid rgba(200,168,255,0.1)',borderRadius:'14px',padding:'4px'}}>
          {tabs.map((t,i)=>(
          ...
          <div>
            <div style={{background:'linear-gradient(135deg,rgba(255,214,160,0.08),rgba(138,90,255,0.06))',border:'1px solid rgba(255,214,160,0.15)',borderRadius:'14px',padding:'14px 18px',marginBottom:'16px',display:'flex',alignItems:'center',justifyContent:'space-between'}}>
              ...
              <div style={{textAlign:'right'}}>
                <div style={{fontStyle:'italic',fontSize:'13px',color:'#C8A8FF'}}>High Aries Energy</div>
                <div style={{fontFamily:'sans-serif',fontSize:'9px',letterSpacing:'2px',color:'rgba(200,168,255,0.4)'}}>Mars · Sun active</div>
              </div>
            </div>

            {[
              {icon:'♀',title:'Love & Relationships',badge:'Venus in Pisces',color:'rgba(255,130,180,0.5)',text:'Venus drifts through Pisces today, casting a dreamy veil over your Aries fire. Soften your edges, beloved. The heart you have been guarding wants to be seen.',tip:'Speak what you feel before midnight. The Waning Moon supports releasing love blocks.'},
              {icon:'☿',title:'Career & Purpose',badge:'Mercury Direct',color:'rgba(255,214,160,0.5)',text:'Mercury moves direct in your 10th house of career, clearing the fog that surrounded your ambitions. Ideas you shelved in February are ready for revival.',tip:'Between 10am and 2pm, Mars fuels your drive. This is your window for bold moves.'},
              {icon:'🌿',title:'Health & Body Wisdom',badge:'Waning Moon',color:'rgba(100,220,130,0.5)',text:'The Waning Gibbous calls your body into rest and restoration. Aries rules the head — you may feel tension at the temples today. Honor the signal.',tip:'Drink warm water with lemon before your breathwork. Place your hands on your heart for 3 breaths after.'},
            ].map(({icon,title,badge,color,text,tip})=>(
              <div key={title} style={{background:'rgba(255,255,255,0.022)',border:'1px solid rgba(200,168,255,0.1)',borderRadius:'16px',padding:'18px',marginBottom:'12px',borderLeft:`3px solid ${color}`}}>
                <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:'10px'}}>
                  ...
                  <span style={{fontFamily:'sans-serif',fontSize:'9px',letterSpacing:'2px',padding:'3px 9px',borderRadius:'10px',background:'rgba(200,168,255,0.08)',border:'1px solid rgba(200,168,255,0.15)',color:'rgba(200,168,255,0.6)'}}>{badge}</span>
                </div>
                <p style={{fontStyle:'italic',fontSize:'14px',letterSpacing:'1px',color:'rgba(220,210,255,0.7)',lineHeight:1.9,marginBottom:'8px'}}>{text}</p>
                <div style={{background:'rgba(138,90,255,0.06)',borderRadius:'8px',padding:'8px 12px',fontFamily:'sans-serif',fontSize:'11px',color:'rgba(200,168,255,0.5)',lineHeight:1.7}}>✦ {tip}</div>
              </div>
            ))}
            ...
            <div style={{background:'linear-gradient(135deg,rgba(255,214,160,0.08),rgba(138,90,255,0.06))',border:'1px solid rgba(255,214,160,0.2)',borderRadius:'16px',padding:'18px',marginBottom:'12px',display:'flex',alignItems:'center',gap:'16px'}}>
              ...
              <div style={{flex:1}}>
                ...
                <div style={{fontStyle:'italic',fontSize:'20px',letterSpacing:'3px',color:'#FFD6A0',marginBottom:'3px'}}>528 Hz · DNA Repair</div>
                <div style={{fontFamily:'sans-serif',fontSize:'11px',color:'rgba(255,214,160,0.5)',lineHeight:1.6}}>Chosen for your Aries Sun under the Waning Gibbous. Perfect for your morning breathwork.</div>
              </div>
            ...
            <div style={{background:'linear-gradient(135deg,rgba(168,232,255,0.06),rgba(60,40,160,0.12))',border:'1px solid rgba(168,232,255,0.15)',borderRadius:'16px',padding:'20px'}}>
              <div style={{display:'flex',alignItems:'center',gap:'12px',marginBottom:'12px'}}>
                <span style={{fontSize:'28px'}}>🌖</span>
                <div>
                  <div style={{fontStyle:'italic',fontSize:'17px',letterSpacing:'3px',color:'#A8E8FF'}}>Moon Ritual · Tonight</div>
                  <div style={{fontFamily:'sans-serif',fontSize:'10px',letterSpacing:'3px',color:'rgba(168,232,255,0.45)'}}>Waning Gibbous · Release</div>
                </div>
              </div>
              <p style={{fontStyle:'italic',fontSize:'14px',color:'rgba(200,230,255,0.7)',lineHeight:1.9,marginBottom:'12px'}}>The Waning Gibbous asks you to release with gratitude. What you have built under the Full Moon has served its purpose. Now it is time to let go.</p>
              {['Light a candle at sunset. Take 3 deep Solar Fire breaths.','Write down 3 things you are releasing — a fear, a habit, a belief.','Place your hands on your heart. Whisper: "I release with love."','Play 528 Hz and sit in stillness for 10 minutes.'].map((step,i)=>(
                <div key={i} style={{display:'flex',alignItems:'flex-start',gap:'10px',marginBottom:'8px'}}>
                  <div style={{width:'20px',height:'20px',borderRadius:'50%',background:'rgba(168,232,255,0.12)',border:'1px solid rgba(168,232,255,0.25)',display:'flex',alignItems:'center',justifyContent:'center',fontStyle:'italic',fontSize:'10px',color:'#A8E8FF',flexShrink:0,marginTop:'1px'}}>{i+1}</div>
                  <div style={{fontFamily:'sans-serif',fontSize:'11px',color:'rgba(168,232,255,0.5)',lineHeight:1.7}}>{step}</div>
                </div>
              ))}
            </div>
          </div>
