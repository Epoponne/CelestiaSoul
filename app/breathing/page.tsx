    ...
    <main style={{background:'#06050E',minHeight:'100vh',color:'#E8E0FF',fontFamily:'Georgia,serif'}}>
      <div style={{maxWidth:'680px',margin:'0 auto',padding:'0 18px 60px'}}>
        ...
        <div style={{background:'rgba(255,255,255,0.05)',borderRadius:'4px',height:'2px',marginBottom:'18px',overflow:'hidden'}}>
          <div style={{height:'100%',background:'linear-gradient(90deg,#8A5AFF,#A8E8FF,#FFD6A0)',borderRadius:'4px',width:`${Math.min((elapsed/1200)*100,100)}%`,transition:'width 1s linear'}}/>
        </div>
        <button onClick={()=>running?stop():start()} style={{width:'100%',padding:'15px',background:'linear-gradient(135deg,rgba(138,90,255,0.25),rgba(255,214,160,0.1))',border:'1px solid rgba(200,168,255,0.35)',borderRadius:'13px',fontStyle:'italic',fontSize:'16px',letterSpacing:'5px',color:'#E8E0FF',cursor:'pointer',marginBottom:'14px'}}>{running?'End Session':'Begin Sacred Breath'}</button>
        <div style={{background:'rgba(255,255,255,0.02)',border:'1px solid rgba(200,168,255,0.08)',borderRadius:'13px',padding:'16px 20px',textAlign:'center'}}>
          <p style={{fontStyle:'italic',fontSize:'11px',letterSpacing:'4px',color:'rgba(155,143,191,0.65)',marginBottom:'8px'}}>✦ Aries ♈ · Today's Affirmation ✦</p>
          <p style={{fontStyle:'italic',fontSize:'14px',color:'#C8A8FF',lineHeight:1.8,margin:0}}>As Aries, ignite your inner fire. Each breath fans the flame of your divine courage.</p>
        </div>
      </div>
    </main>
  )
