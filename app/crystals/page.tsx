    ...
    <main style={{background:'#06050E',minHeight:'100vh',color:'#E8E0FF',fontFamily:'Georgia,serif'}}>
      <div style={{maxWidth:'680px',margin:'0 auto',padding:'0 18px 100px'}}>
        ...
        <div style={{display:'flex',gap:'8px',marginBottom:'24px',overflowX:'auto',paddingBottom:'4px'}}>
          {categories.map(c=>(
            <div key={c.id} onClick={()=>setFilter(c.id)} style={{padding:'8px 16px',borderRadius:'20px',cursor:'pointer',fontFamily:'sans-serif',fontSize:'11px',letterSpacing:'2px',background:filter===c.id?'rgba(138,90,255,0.3)':'rgba(255,255,255,0.03)',border:`1px solid ${filter===c.id?'rgba(200,168,255,0.5)':'rgba(200,168,255,0.1)'}`,color:filter===c.id?'#E8E0FF':'rgba(200,168,255,0.4)',whiteSpace:'nowrap',flexShrink:0}}>
              {c.emoji} {c.label}
            </div>
          ))}
        </div>
        ...
        {selected && (
          <div style={{background:'linear-gradient(135deg,rgba(138,90,255,0.12),rgba(40,20,100,0.2))',border:`2px solid ${selected.border||'rgba(200,168,255,0.2)'}`,borderRadius:'20px',padding:'28px',marginBottom:'24px',boxShadow:`0 0 30px ${selected.bg}`}}>
            ...
            <div style={{background:selected.bg,border:`1px solid ${selected.color}`,borderRadius:'12px',padding:'14px',textAlign:'center'}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:'12px'}}>
          {filtered.map(c=>(
            <div key={c.name} onClick={()=>setSelected(c)} style={{background:c.bg,border:`1px solid ${c.border||'rgba(200,168,255,0.2)'}`,borderRadius:'16px',padding:'20px 12px',textAlign:'center',cursor:'pointer',transition:'all 0.2s',boxShadow:selected?.name===c.name?`0 0 20px ${c.bg}`:'none',transform:selected?.name===c.name?'scale(1.02)':'scale(1)'}}>
